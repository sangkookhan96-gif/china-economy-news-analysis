...
CATEGORIES = {
    ...
    '거시경제': [
        # 기존
        '经济', '增长', '消费', '投资', '储蓄', '人口', '劳动',
        # 재정 정책
        '财政', '财政政策', '预算', '赤字', '国债', '地方债', '减税', '税收',
        # 통화 정책
        '货币', '货币政策', '人民银行', '央行', 'LPR', 'MLF', '降息', '降准', '加息', '利率', '存款准备金',
        # 환율
        '汇率', '人民币汇率', '外汇储备', '中间价', '离岸人民币',
        # 거시 지표
        'CPI', 'PPI', 'PMI', 'GDP', '通货膨胀', 'M2', '社会融资', '居民收入',
    ],
...
MACRO_KEYWORDS = [
    # 재정 (중국 정부 전용 용어)
    '财政政策', '财政赤字', '地方债', '专项债', '减税降费',
    # 통화 (중국 인민은행 전용 — 外国 央行에 쓰이는 '央行'·'货币政策' 제외)
    '人民银行', 'LPR', 'MLF', '降准', '存款准备金率', '逆回购', '公开市场操作',
    # 환율 (위안화 전용)
    '人民币汇率', '外汇储备', '中间价', '离岸人民币',
    # 중국 전용 거시 지표
    '社会融资规模', '新增贷款', 'M2增速', 'GDP增速',
]
...
def balance_categories(news_list: list, target_count: int = 10, max_local_gov: int = 1) -> list:
    ...
    selected = []
    # 우선순위 순서: 과학기술·산업 최우선 → 거시경제·대외경제 상향 → 기업·금융·정책
    main_categories = ['과학기술', '산업', '에너지', '거시경제', '대외경제', '기업', '금융', '정책']
