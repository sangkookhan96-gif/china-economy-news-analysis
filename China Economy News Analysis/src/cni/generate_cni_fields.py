...
def generate_enhanced(news_id, original_title, original_content, enable_papago=True):
    ...
    if enable_papago:
        ...
        # 규칙은 발행·승인된 헤드라인 877건(약 2개월) 스타일 분석에서 도출:
        # 명사(체언) 종결 90%, 숫자 76%, 쉼표 절구조 70%, 길이 중앙 31·평균 31자.
        _HL_KO_PROMPT = (
            "한국어 뉴스 요약에서 핵심 사실을 추출하여 헤드라인을 작성하라.\n\n"
            "규칙:\n"
            "1. 길이는 32자 내외, 최대 40자 (공백 포함)\n"
            "2. 한국어만 사용 (중국어 한자 절대 금지)\n"
            "3. 구조: \"주체(기업/기관명), 핵심 수치 + 결과\" — 쉼표로 절을 구분\n"
            "4. 요약 안에 있는 수치를 반드시 1개 이상 포함\n"
            "5. 반드시 명사(체언)로 끝낼 것 — 예: …발표, …돌파, …증가, …감소, "
            "…계획, …예정, …개최, …초과. '~했다/~한다/~된다' 같은 서술형 종결과 "
            "'~로/~며/~을/~에' 같은 조사 종결은 절대 금지\n"
            "6. 요약에 없는 정보를 추가하지 말 것\n\n"
            "좋은 예시:\n"
            ...
            "나쁜 예시 (금지):\n"
            "- 중국 정부, 산업 발전 새로운 추진  (수치 없음·모호)\n"
            "- 과학기술과 산업 혁신의 깊은 융합  (수치 없음·모호)\n"
            "- 엔비디아, 신형 칩 수요가 왕성하다  (서술형 종결)\n\n"
            "요약:\n{summary}\n\n"
            "40자 이내, 명사로 끝나는 한국어 헤드라인만 출력:"
        )
